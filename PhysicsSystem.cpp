...
void PhysicsSystem::BroadPhaseBppTree() {
	...
	int min_z_value = std::numeric_limits<int>::max();
	...
	for (i = 0; i < n->num_keys && n->keys[i] < min_z_value; i++) // getting the key within the node if exists
		;
	if (i == n->num_keys)
